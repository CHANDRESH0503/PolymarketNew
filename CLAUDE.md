...
**Correlation-aware Kelly — ✅ built (`sizing.correlation_kelly`)**
...
covariance Σ from an equicorrelation `rho`) shrinks the correlated legs. Enable
with `CORR_KELLY=1` / `CORR_KELLY_RHO`.
...
src/
  ...
  strategy/
    edge.py              edge detection + tradability filter (+ NOWCAST path)
